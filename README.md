...
Optional:
- Add `GEMINI_MODEL` as a secret (or edit the workflow env) to force a specific model.
...
- **Gemini quota / invalid key**:
  - The logs in the “Generate puzzles” step will show the API error.
...
This repo’s client code will now **fail-soft** (fallback to `localStorage`) if Firestore rejects requests, but you’ll still want to correct the underlying Firebase config.
